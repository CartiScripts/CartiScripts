...
local page = ulib:addPage("Carti Fusion", 5012544693)
...
function magBall(ball)
   if ball and player.Character then
       ...
       task.wait()
       firetouchinterest(player.Character["Left Arm"], ball, 0.5)
       firetouchinterest(player.Character["Right Arm"], ball, 0.5)
   end
end
section1:addTextbox("Niggers ARE NOT Welcomed", nil, function()end)
section1:addToggle("Nigger MAG", nil, function(bool)
    shared.Mags = bool
